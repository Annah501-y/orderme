...
class OrderController extends Controller
{
        ...
        */
        ...
        foreach ($sellerGroups as $sellerId => $sellerItems) {
            ...
            if (! $sellerAddress) {
                return response()->json([
                    'success' => false,
                    'message' => "Seller {$sellerId} has not added a delivery address.",
                ], 422);
            }
            ...
            try {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Unable to calculate delivery for one of the sellers.',
                ], 422);
            }
        ...
        $order = DB::transaction(function () use (
            ...
            foreach ($sellerDeliveryData as $sellerId => $deliveryData) {
                SellerOrder::create([
                    ...
                    'seller_id' => $sellerId,
                    'status' => 'pending',
                    'seller_total' => $deliveryData['seller_subtotal'],
    ...
    public function calculateDeliveryFee(
        ...
        if ($selectedItems->isEmpty()) {
            return response()->json([
                'message' => 'The selected cart items could not be found.',
            ], 422);
        }
        ...
        foreach ($selectedItems as $cartItem) {
            ...
            if (
                ...
                return response()->json([
                    'message' => "Product {$cartItem->product->name} is unavailable or has insufficient stock.",
                ], 422);
            }
        ...
        foreach ($sellerGroups as $sellerId => $sellerItems) {
            ...
            if (! $sellerAddress) {
                return response()->json([
                    'message' => "Seller {$seller->name} does not have a saved delivery address.",
                ], 422);
            }
            ...
            $sellerSubtotal = $sellerItems->sum(
                fn ($cartItem) => (float) $cartItem->product->price *
                    (int) $cartItem->quantity
            );
            ...
            $deliveryBreakdown[] = [
                ...
                'distance_km' => $route['distance_km'],
                'duration_minutes' => $route['duration_minutes'],
                'vehicle_type' => $feeData['vehicle_type'],
                'rate_per_km' => $feeData['rate_per_km'],
    ...
    public function cancel(Request $request, Order $order): JsonResponse
    {
        ...
        if (! in_array($order->status, $allowedStatuses, true)) {
            return response()->json([
                'success' => false,
                'message' => "Order cannot be cancelled while its status is {$order->status}.",
            ], 422);
        }
    ...
    public function sellerUpdateStatus(
        ...
        if (! in_array(
            ...
            return response()->json([
                'success' => false,
                'message' => "Cannot change seller order status from {$currentStatus} to {$newStatus}.",
            ], 422);
        }
    ...
    public function payment(
        ...
        if (! in_array(
            ...
            return response()->json([
                'success' => false,
                'message' => "Payment cannot be made while the order status is {$order->status}.",
            ], 422);
        }
